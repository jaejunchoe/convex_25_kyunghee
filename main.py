...
def create_visualizations(aggregated, timestamp):
    ...
    plt.tight_layout()
    plt.savefig(f'comparison_bars_{timestamp}.png', dpi=150)
    ...
    plt.tight_layout()
    plt.savefig(f'comparison_boxplots_{timestamp}.png', dpi=150)
    ...
    plt.tight_layout()
    plt.savefig(f'improvement_percentage_{timestamp}.png', dpi=150)
    print(f"Saved improvement chart")

def main():
    print("="*80)
    print("Parking Allocation (NO Distance Constraint)")
    print("="*80)
    
    data_path = os.path.join('data', 'summary_weekday_08.csv')
    n_users = 1000
    n_runs = 20
    ...
    with pd.ExcelWriter(f'academic_report_{timestamp}.xlsx') as writer:
        summary_data = []
    ...
    print(f"\n{'='*80}")
    print("RESULTS SUMMARY (NO Distance Constraint)")
    print(f"{'='*80}\n")
    ...
    print(f"\n{'='*80}")
    print(f"Report saved to: academic_report_{timestamp}.xlsx")
    print(f"{'='*80}\n")
    print("Done!")
